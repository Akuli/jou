...
static noreturn void fail_with_parse_error(const Token *token, const char *what_was_expected_instead)
{
    ...
    case TOKEN_DOUBLE:
        ...
        break;
    case TOKEN_CHAR:
        strcpy(got, "a character");
        break;
    case TOKEN_STRING:
        strcpy(got, "a string");
        break;
    case TOKEN_OPERATOR:
        snprintf(got, sizeof got, "'%s'", token->data.operator);
        break;
    case TOKEN_NAME:
        snprintf(got, sizeof got, "a variable name '%s'", token->data.name);
        break;
    case TOKEN_NEWLINE:
    ...
    case TOKEN_DEDENT:
        ...
        break;
    case TOKEN_KEYWORD:
        snprintf(got, sizeof got, "the '%s' keyword", token->data.name);
        break;
    }
...
static AstType parse_type(ParserState *ps)
{
    ...
    if (ps->tokens->type != TOKEN_NAME && !is_keyword(ps->tokens, "None") && !is_keyword(ps->tokens, "void") && !is_keyword(ps->tokens, "noreturn") && !is_keyword(ps->tokens, "short") && !is_keyword(ps->tokens, "int") && !is_keyword(ps->tokens, "long") && !is_keyword(ps->tokens, "byte") && !is_keyword(ps->tokens, "float") && !is_keyword(ps->tokens, "double") && !is_keyword(ps->tokens, "bool"))
    {
    ...
    while (is_operator(ps->tokens, ".") || is_operator(ps->tokens, "->") || is_operator(ps->tokens, "["))
    {
        ...
        else
        {
            ...
            *obj = result;
            ...
            if (is_deref && is_call)
                result.kind = AST_EXPR_DEREF_AND_CALL_METHOD;
            if (is_deref && !is_call)
                result.kind = AST_EXPR_DEREF_AND_GET_FIELD;
            if (!is_deref && is_call)
                result.kind = AST_EXPR_CALL_METHOD;
            if (!is_deref && !is_call)
...
static AstExpression parse_expression_with_unary_operators(ParserState *ps)
{
    ...
    while (is_operator(ps->tokens, "++") || is_operator(ps->tokens, "--") || is_operator(ps->tokens, "&") || is_operator(ps->tokens, "*") || is_keyword(ps->tokens, "sizeof"))
        ps->tokens++;
...
// Get the correct data from the token
static void get_data_from_token(char data[100], const Token *token) {
    switch (token->type)
    {
    case TOKEN_SHORT:
        sprintf(data, "%hd", (short)token->data.short_value);
        break;
    case TOKEN_INT:
        sprintf(data, "%d", (int)token->data.int_value);
        break;
    case TOKEN_LONG:
        sprintf(data, "%lld", (long long)token->data.long_value);
        break;
    case TOKEN_CHAR:
        data[0] = token->data.char_value;
        break;
    case TOKEN_STRING:
        strcpy(data, token->data.string_value);
        break;
    case TOKEN_FLOAT:
    case TOKEN_DOUBLE:
    case TOKEN_KEYWORD:
    case TOKEN_NAME:
        strcpy(data, token->data.name);
        break;
    case TOKEN_OPERATOR:
        strcpy(data, token->data.operator);
        break;
    default:
        // TOKEN_NEWLINE TOKEN_DEDENT TOKEN_INDENT TOKEN_END_OF_FILE
        break;
    }
}

// collect all the data and combine them into a string
static char* get_assertion_from_tokens(ParserState *ps)
{
    List(char) cond = {0};
    int cnt = 0;
    do
    {
        ...
        cnt++;
        get_data_from_token(data, ps->tokens);
        trim_whitespace(data);

        if (data[0] != ')' && cond.len >= 1 && cond.ptr[cond.len - 1] != '(') {
            AppendStr(&cond, " ");
        }
        AppendStr(&cond, data);
...
static AstStatement parse_oneline_statement(ParserState *ps)
{
    ...
    else if (is_keyword(ps->tokens, "assert"))
    {
        ...
        result.data.assertion.condition_str = get_assertion_from_tokens(ps);
