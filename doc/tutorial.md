...
If you don't use `as`, the value will not wrapp around, and you will instead get a compiler error:
...
As the name suggests, `int64` is 64-bit instead of 32-bit, so twice the size of an `int`,
and hence much less likely to wrap around.
To print an `int64`, [use `%lld` instead of `%d`](types.md#integers).
...
We are using `%lld`, because `strlen()` returns a `long` (same as `int64`).
