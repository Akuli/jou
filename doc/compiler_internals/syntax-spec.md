...
- **Double literals** look like `12.` or `12.34` or `123.456e5` or `1e-5`.
    There are two kinds of double literals:
    1. Zero or more digits (0-9), then `.`, then zero or more digits.
        For example, `12.34` or `.5` or `5.`.
        Just `.` is not a valid double literal: there must be at least one digit somewhere.
    2. First a kind 1 double literal or just one or more digits,
