...
for a_file in $(grep -o 'link "libs/.*\.a"' compiler/llvm.jou | cut -d'"' -f2); do
    if [ -f $a_file ]; then
        echo "  $a_file has already been generated."
    else
        echo "  Generating $a_file..."
        name="$(basename $a_file .a)"   # libs/libFoo.a --> libFoo
        cd libs
